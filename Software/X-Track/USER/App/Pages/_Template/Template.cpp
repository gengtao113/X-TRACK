...
Template::Template()
    : timer(nullptr)
{
    base.ops = &PageOpsBinder<Template>::ops;
}
...
{
    LV_LOG_USER("begin");
    Page_SetCustomCacheEnable(&base, true);
    Page_SetCustomLoadAnimType(&base, PageManager::LOAD_ANIM_OVER_BOTTOM, 1000, lv_anim_path_bounce);
}
...
void Template::onViewLoad()
{
    ...
    lv_label_set_text(View.ui.labelTitle, base._Name);
...
void Template::onViewWillAppear()
{
    ...
    lv_obj_set_style_bg_color(base._root, param.color, LV_PART_MAIN);
    lv_obj_set_style_bg_opa(base._root, LV_OPA_COVER, LV_PART_MAIN);
